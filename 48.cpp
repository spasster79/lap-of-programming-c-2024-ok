...
private:
    ...
    std::string triggerDeviceID_; // ID of the device that triggers the rule
    std::function<bool(const AbstractSmartDevice& device)> condition_; // Condition to check on triggerDevice
    std::string actionDeviceID_;  // ID of the device to perform action on
    // Action takes the device to act on, and a controller reference if needed for context/other actions
    std::function<void(AbstractSmartDevice& device_to_act_on, SmartHomeController& controller)> action_; 

public:
    AutomationRule(const std::string& name,
                   const std::string& triggerDevID,
                   std::function<bool(const AbstractSmartDevice&)> condition,
                   const std::string& actionDevID,
                   std::function<void(AbstractSmartDevice&, SmartHomeController&)> action,
                   char uid_prefix = 'A'); // 'AR' for rule, but UID takes char

    std::string getRuleIDString() const;
    std::string getRuleName() const;
    std::string getTriggerDeviceID() const;
    std::string getActionDeviceID() const;

    // Evaluates the rule's condition against the provided trigger device
    bool evaluate(const AbstractSmartDevice& triggerDevice) const;

    // Executes the rule's action on the provided action device
    void execute(AbstractSmartDevice& actionDevice, SmartHomeController& controller) const;
};
...
private:
    ...
    std::vector<AutomationRule> automation_rules_; // 新增, new

    // Private constructor and destructor for Singleton
    SmartHomeController();
    ~SmartHomeController(); // Important for cleaning up instance_ if it's newed by getInstance

public:
    static SmartHomeController* getInstance();
    static void cleanupInstance(); // To explicitly delete the singleton instance

    SmartHomeController(const SmartHomeController&) = delete;
    SmartHomeController& operator=(const SmartHomeController&) = delete;

    // Device Management
    // Using specific parameters for known device types for now.
    // A more advanced system might use a factory or map of parameters.
    bool addDevice(const std::string& name, const Location& loc, const std::string& device_type,
                   double param1_val = 0.0, const std::string& param_str_val = "", double param2_val = 0.0);
    ...
    User* findUserByID(const std::string& user_id_string); // Returns raw pointer to User in vector

    // Operation Execution
    bool executeDeviceOperation(const std::string& userID_str,
                                const std::string& deviceID_str,
                                const std::string& operation_details);
    // (Chinese) 自動化規則管理
    // (English) Automation Rule Management
    void addAutomationRule(const AutomationRule& rule);
    // (Chinese) 處理裝置狀態改變，檢查並執行規則
    // (English) Process device state change, check and execute rules
    void processDeviceStateChange(const std::string& changed_device_id_string);
};

//TEMPLATE END

//APPEND BEGIN
// --- Helper Function Templates (can be in a separate utility file or AutomationRule.cpp) ---
// Condition: Check if device is ON
std::function<bool(const AbstractSmartDevice& dev)> conditionIsOn = 
    [](const AbstractSmartDevice& dev) {
        std::cout << "  [Condition Check] Device '" << dev.getName() << "' (ID: " << dev.getDeviceIDString() 
                  << ") is " << (dev.isOn() ? "ON" : "OFF") << ". Condition (is ON) is " << (dev.isOn() ? "MET." : "NOT MET.") << std::endl;
        return dev.isOn();
    };

// Condition: Check if device is OFF
std::function<bool(const AbstractSmartDevice& dev)> conditionIsOff = 
    [](const AbstractSmartDevice& dev) {
        std::cout << "  [Condition Check] Device '" << dev.getName() << "' (ID: " << dev.getDeviceIDString() 
                  << ") is " << (dev.isOn() ? "ON" : "OFF") << ". Condition (is OFF) is " << (!dev.isOn() ? "MET." : "NOT MET.") << std::endl;
        return !dev.isOn();
    };

// Action: Turn ON a device
std::function<void(AbstractSmartDevice& dev_to_act_on, SmartHomeController&)> actionTurnOn =
    [](AbstractSmartDevice& dev_to_act_on, SmartHomeController& /*controller*/) {
        std::cout << "  [Action] Turning ON device '" << dev_to_act_on.getName() << "' (ID: " << dev_to_act_on.getDeviceIDString() << ")" << std::endl;
        dev_to_act_on.turnOn();
    };

// Action: Turn OFF a device
std::function<void(AbstractSmartDevice& dev_to_act_on, SmartHomeController&)> actionTurnOff =
    [](AbstractSmartDevice& dev_to_act_on, SmartHomeController& /*controller*/) {
        std::cout << "  [Action] Turning OFF device '" << dev_to_act_on.getName() << "' (ID: " << dev_to_act_on.getDeviceIDString() << ")" << std::endl;
        dev_to_act_on.turnOff();
    };

// Action: Set Thermostat Target Temperature
std::function<void(AbstractSmartDevice& dev_to_act_on, SmartHomeController&)> actionSetThermoTarget(double temp) {
    return [temp](AbstractSmartDevice& dev_to_act_on, SmartHomeController& /*controller*/) {
        if (auto* thermo = dynamic_cast<ThermostatDevice*>(&dev_to_act_on)) {
            std::cout << "  [Action] Setting thermostat '" << thermo->getName() << "' target to " << temp << "C." << std::endl;
            thermo->setTargetTemperature(temp);
        } else {
            std::cerr << "  [Action Error] Device " << dev_to_act_on.getDeviceIDString() << " is not a ThermostatDevice for setTargetTemperature." << std::endl;
        }
    };
}
// --- End Helper Function Templates ---

void printDeviceStatus(const std::string& id, SmartHomeController* controller) {
    auto dev = controller->findDeviceByID(id);
    if (dev) {
        std::cout << "  Status of " << dev->getName() << " (ID: " << id << "): " << dev->getStatusString() << std::endl;
    } else {
        std::cout << "  Device " << id << " not found for status check." << std::endl;
    }
}
...
int main() {
    ...
    // --- Setup Users, Devices, Rooms (simplified from Stage 7 example) ---
    controller->registerUser("RuleTester", UserAccessLevel::ADMIN); // U-001
    
    Location livingRoomLoc("Living Room");
    controller->addDevice("LivingRoomLight", livingRoomLoc, "LightDevice", 0, "White"); // L-002
    controller->addDevice("LivingRoomThermo", livingRoomLoc, "ThermostatDevice", 20.0, "", 18.0); // T-003

    Location kitchenLoc("Kitchen");
    controller->addDevice("KitchenFan", kitchenLoc, "LightDevice", 0, "White"); // L-004 (Treating fan as a light for simplicity)


    std::cout << "\n--- Initial Device Statuses ---" << std::endl;
    printDeviceStatus("L-002", controller);
    printDeviceStatus("T-003", controller);
    printDeviceStatus("L-004", controller);


    // --- Define and Add Automation Rules ---
    std::cout << "\n--- Defining and Adding Automation Rules ---" << std::endl;
    // Rule 1: If LivingRoomLight (L-002) turns ON, set LivingRoomThermo (T-003) target to 22.0C
    AutomationRule rule1("LR Light ON -> Thermo Cool", "L-002", conditionIsOn, "T-003", actionSetThermoTarget(22.0));
    controller->addAutomationRule(rule1);
    std::cout << "Added Rule: " << rule1.getRuleName() << " (ID: " << rule1.getRuleIDString() << ")" << std::endl;

    // Rule 2: If LivingRoomLight (L-002) turns OFF, turn KitchenFan (L-004) ON
    AutomationRule rule2("LR Light OFF -> Kitchen Fan ON", "L-002", conditionIsOff, "L-004", actionTurnOn);
    controller->addAutomationRule(rule2);
    std::cout << "Added Rule: " << rule2.getRuleName() << " (ID: " << rule2.getRuleIDString() << ")" << std::endl;
    std::cout << std::endl;


    // --- Simulate Device State Changes to Trigger Rules ---
    std::cout << "--- Simulating Device State Changes ---" << std::endl;

    // 1. Turn LivingRoomLight ON (should trigger Rule 1)
    std::cout << "\n>>> Scenario 1: Turning LivingRoomLight (L-002) ON <<<" << std::endl;
    controller->executeDeviceOperation("U-001", "L-002", "turn_on"); // User U-001 turns it on
    printDeviceStatus("L-002", controller); // Verify light is on
    std::cout << "Processing state change for L-002..." << std::endl;
    controller->processDeviceStateChange("L-002"); // Controller checks rules for L-002
    printDeviceStatus("T-003", controller); // Check thermostat status
    printDeviceStatus("L-004", controller); // Check fan status (should be unchanged)


    // 2. Turn LivingRoomLight OFF (should trigger Rule 2)
    std::cout << "\n>>> Scenario 2: Turning LivingRoomLight (L-002) OFF <<<" << std::endl;
    controller->executeDeviceOperation("U-001", "L-002", "turn_off");
    printDeviceStatus("L-002", controller); // Verify light is off
    std::cout << "Processing state change for L-002..." << std::endl;
    controller->processDeviceStateChange("L-002"); // Controller checks rules for L-002
    printDeviceStatus("T-003", controller); // Check thermostat status (should be unchanged by this rule)
    printDeviceStatus("L-004", controller); // Check fan status (should be ON)

    std::cout << "\n--- End of main function ---" << std::endl;
    SmartHomeController::cleanupInstance();
    return 0;
}
//APPEND END